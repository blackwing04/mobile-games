...
原始 21 張完成後，劇本流程上又需要 2 張：

### 22. `scene_phone_static` — 電話雜訊刺耳

**情境**：妹妹剛從詭異夢境中驚醒，雙手顫抖地按下哥哥的號碼。撥號音響了兩聲後通話接通，但聽筒裡不是哥哥的聲音，而是一陣刺耳的電子雜訊。她隱約聽見背景有什麼聲音在說話，被雜訊撕成碎片。雜訊瞬間放大成一聲尖銳嘯叫，妹妹痛得把手機從耳邊扯開。

**構圖**：近景，妹妹**側臉特寫**（半身或胸上半身），**手機被她扯離耳邊的瞬間**，表情扭曲、單眼閉著、皺眉、嘴微張的痛苦表情。從手機聽筒射出**鋸齒狀、刺耳的視覺化雜訊波形**（用破碎墨線 + 衝擊感放射線從手機向外擴散）

**色塊處理**：背景大塊黑/冷灰、妹妹白襯衫米白色塊、頭髮純黑、紅色鋸齒雜訊線是畫面唯一彩色
...
### 23. `scene_monster_glimpse` — 教室後排怪物剪影
...
**色塊處理**：教室大量純黑色塊填滿（背景吞噬感）、米白手電筒光柱、怪物剪影是純黑墨水暈染感、無血鏽紅（這張是純黑白二元）
...
全部 23 張完成後，請：
1. 確認所有檔案命名正確（`scene_xxx.webp` 對應 23 個 ID — `scene_door_stuck_impatient` / `scene_endless_stairs_search` / `scene_phone_again_clock` / `scene_phone_again_outgoing` / `scene_final_ascent_sms` / `scene_final_ascent_call` 共用主版圖，不單獨交付）
2. 整理成一個壓縮檔（zip）
